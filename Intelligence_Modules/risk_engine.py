...
class RiskEngine:
    ...
    @staticmethod
    def calculate_weighted_risk(video_path):
        """
        Feature 1: Weighted Risk Probability Scoring.
        Returns: risk_score (0-100), details (dict)
        Lower is better/safer. High score = High Risk.
        """
        details = {}
        risk_score = 0.0
        
        # 1. Scene Count (Weight: 30%)
        scenes = RiskEngine.check_scene_count(video_path)
        ...
        # 3. File Size (Weight: 20%) - Proxy for encoded complexity
        try:
            size_mb = os.path.getsize(video_path) / (1024*1024)
            details['size_mb'] = f"{size_mb:.1f}"
            if size_mb < 2.0: # Suspiciously small
                risk_score += 20
            elif size_mb < 5.0:
                risk_score += 5
        except: pass
        
        # 4. Duration (Weight: 10%) - Too short is risky
        # (Assuming we have duration, if not we skip or use ffmpeg)
        # For now, we skip duration check inside this static method to avoid extra I/O
        
        details['final_score'] = risk_score
