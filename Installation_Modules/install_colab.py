...
def install_deps():
    # 1. System Deps (FFmpeg + espeak-ng for Kokoro)
    if shutil.which("ffmpeg") is None or shutil.which("espeak-ng") is None:
        try:
            # APT-GET requires shell=True for && chaining
            run_cmd("apt-get update && apt-get install -y ffmpeg espeak-ng", "Installing System Dependencies (FFmpeg, espeak-ng)", check=False, use_shell=True)
        except Exception:
             log("Could not install system dependencies via apt-get.", "WARN")
    
    # 2. Upgrade pip in venv
    run_cmd([PYTHON_EXEC, "-m", "pip", "install", "--upgrade", "pip"], "Upgrading pip")

    # 3. Install Python Deps
    # Updated: Look in Datasets_and_text_files/requirements.txt if root requirements.txt is missing
    req_file = "requirements.txt"
    if not os.path.exists(req_file):
        req_file = os.path.join("Datasets_and_text_files", "requirements.txt")
...
def main():
    ...
    tools_script = os.path.join("Installation_Modules", "tools-install.py")
    if os.path.exists(tools_script):
        log(f"Running {tools_script}...", "ACTION")
        run_cmd([PYTHON_EXEC, tools_script], "Installing Heavy Models", check=False)
