...
class TextOverlay:
    ...
    def _escape_drawtext(self, text: str) -> str:
        ...
        text = text.replace("\\", "\\\\")
        text = text.replace(":", "\\:")
        # Robust Fix: Remove apostrophes or replace with safe smart quote to prevent filter-chain breakage
        text = text.replace("'", "") 
        text = text.replace("%", "\\%")
...
"""
    ...
    def add_overlay(self, video_path, output_path, text, lane="caption", size=60):
        ...
        if lane == "caption":
             wrapped_text = self._wrap_text(text, max_chars=32) # Increased to 32 to prevent "Curves that" splits
        else:
    ...
    def _apply_drawtext(self, video_path, output_path, text, lane, size):
        ...
        if lane == "fixed":
             ...
             # Anchor Point = h*0.92 (Safe above branding)
             ...
             for i, line in enumerate(lines):
                 ...
                 y_expr = f"(h*0.92) - ({total_lines - i} * {line_height}) - 20"
...
def get_timed_overlay_filter(text, lane, start, duration, width=1080, height=1920, size=60, color="white"):
    """
    ...
    # Enable expression
    enable_expr = f"between(t,{float(start):.3f},{float(start)+float(duration):.3f})"
    ...
    for i, line in enumerate(lines):
        ...
        y_expr = f"h*0.5" # Default

        if lane == "caption":
                # caption -> grows upward from h*0.92
                # padding added (- 20) to maintain gap from branding
                y_expr = f"(h*0.92)-({total_lines-i}*{line_height})-20"
        
        elif lane == "branding_upper":
